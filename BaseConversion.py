def convert_to_10_with_alphabet(s, alphabet):
    n = len(alphabet)
    if isinstance(s, str):
        v = 0
        for c in s:
...
def convert_from_10_to_any_with_alphabet(num, alphabet):
    base = len(alphabet)
    ret = []
    while num:
...
def convert(input, source, target):
    a = convert_to_10_with_alphabet(input, source)
    return convert_from_10_to_any_with_alphabet(a, target)
